...
function hideParamsPanel() {
    if (els.paramsEmpty) els.paramsEmpty.classList.remove('hidden');
...
function selectModelChip(chip, dataEl) {
    ...
    if (els.paramsEmpty) {
        ...
        els.paramsEmpty.classList.toggle('hidden', !!(cfg && cfg.params.length > 0));
    }
...
function initPromptCounter() {
    ...
    els.configPrompt.addEventListener('input', () => {
        const words = els.configPrompt.value.trim().split(/\s+/).filter(Boolean).length;
